...
Locked OPT-1.3B design (4,700 fresh paths)
-------------------------------------------
* T=600 and 20 frozen prompts.
* Temperatures 0.75 and 1.0 only, with no top-k/top-p truncation and no EOS stop.
* Two-region grid: L in {50,100,200}, G in {10,50,100}; 200 paths/cell/temp,
  except L=200,G=50 has 500 paths/cell/temp.
* Four-region stress case: L=50,G=25; 200 paths/temp.
* All-null implementation audit: 50 paths/temp.
...
PATHS_PER_TEMPERATURE = 2_350
TOTAL_MODEL_PATHS = 4_700
    ...
    for temperature in temperatures:
        for length in (50, 100, 200):
            for gap in (10, 50, 100):
                # The L=200,G=50 cell includes the predeclared top-up to 500.
                n_paths = 500 if (length, gap) == (200, 50) else 200
                schedule = two_region_schedule(length, gap)
                scenarios.append(
                    ScenarioSpec(
        ...
        null_schedule = ScheduleSpec("all_null", ())
        scenarios.append(
            ScenarioSpec(
                scenario_id=f"audit_all_null_temp{temperature_label(temperature)}",
...
def study_scenarios() -> list[ScenarioSpec]:
    ...
    if sum(s.n_paths for s in scenarios) != TOTAL_MODEL_PATHS:
        raise AssertionError("study manifest must contain exactly 4,700 paths")
    return scenarios
